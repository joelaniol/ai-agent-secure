# Build AI Agent Secure GUI.
# Reads Bash scripts, embeds them as C# strings, and compiles into dist/.
...
function Assert-BashSyntax {
    ...
    if (-not $BashPath) {
        throw "Kein bash gefunden. Der Build-Check für eingebettete Shell-Scripts kann nicht ausgeführt werden."
    }

    $normalized = Convert-ToBashPath -FilePath $FilePath
    & $BashPath --noprofile --norc -lc "bash -n '$normalized'"
    if ($LASTEXITCODE -ne 0) {
        throw "Bash-Syntaxprüfung fehlgeschlagen: $FilePath"
    }
}
...
function Remove-ExistingBuildOutput {
    ...
    if (-not $target.StartsWith($dist + [System.IO.Path]::DirectorySeparatorChar, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Build-Artefakt liegt außerhalb von dist/: $target"
    }
...
# Since the slice refactor the protection layer lives in lib/protection-*.sh;
# lib/protection.sh is only a loader for dev/test. Build and install concatenate
# the slices in this order into one protection.sh, matching the runtime install
# shape from before the refactor.
# Order matters: core provides variables and helpers used by every other slice;
# tokenize follows because delete and ps use the token streams.
$protectionSlices = @(
...
# ── Generate EmbeddedScripts.cs ──
$embedded = @"
// Auto-generated by build-gui.ps1 - do not edit manually
...
# Canonical name: matches README/release download. Remove old artifacts before
# compiling so a failed build cannot be mistaken for a fresh result. If the EXE
# is still running (file lock), fall back to a suffixed name so the build can
# finish.
...
if (-not (Remove-ExistingBuildOutput -Path $out -DistDir $distDir)) {
    ...
    $out = $fallbackOut
    if (-not (Remove-ExistingBuildOutput -Path $out -DistDir $distDir)) {
        throw "Build-Ziel ist gesperrt. Bitte laufende AI Agent Secure GUI schließen: $out"
    }
    Write-Host "Alte EXE läuft noch, baue als $outName" -ForegroundColor Yellow
}
